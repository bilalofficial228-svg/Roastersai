...
const LANGUAGE_DIRECTIONS: Record<Language, string> = {
  ...
  roman_urdu:
    "Generate roast in ROMAN URDU ONLY — Pakistani/Urdu speaking style written entirely in English alphabet (Roman script). ZERO Urdu script, ZERO Devanagari. Write exactly like Pakistanis text on WhatsApp: mix Urdu words in Roman letters naturally with English. Use Pakistani desi humor — Karachi/Lahore/Islamabad references, load shedding, rishta aunty, shaadi pressure, chai, jugaar. Words to use: 'bhai', 'yaar', 'matlab', 'teri life', 'tera', 'hai', 'lekin', 'kyunki', 'isliye', 'aur', 'mein', 'se', 'ko'. STRICT STRUCTURE — 3 lines MAX: Line 1 = NAME + CITY (city name in Roman) + JOB joke. Line 2 = weakness or status burn with local cultural reference. Line 3 = killer punchline about being single/relationship status. 2-3 emojis inside sentences. GOOD example: 'Bhai tu Karachi mein engineer hai / lekin teri life ka code bahut buggy hai 💀 / Single isliye hai kyunki tera WiFi se bhi kam connection hai 😂🔥'. SAVAGE example: 'Ahmed, Lahore mein doctor hai / lekin apni zindagi ki diagnosis / kabhi sahi nahi ki 💀 / Akela isliye hai kyunki rishta / aunty bhi teri kundali dekh ke bhaag gayi 🔥'. Always funny, always clear, always screenshot worthy!",
};
